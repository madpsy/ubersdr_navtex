...
struct DecoderStats {
    enum class State { SYNC_SETUP = 0, SYNC = 1, READ_DATA = 2 } state;

    // Signal confidence: magnitude of the prompt bit accumulator average.
    // Higher = cleaner signal. Only meaningful when state == READ_DATA.
    double signal_confidence;

    // Rolling window character decode quality (last N characters).
    // Only updated while state == READ_DATA; reset on SYNC_SETUP entry.
    int chars_clean;   // decoded directly from alpha
    int chars_fec;     // recovered via FEC (rep copy or bit-flip)
    int chars_failed;  // hard failure — character lost
};
...
private:
    ...
    // Rolling window for decode quality metrics (last N characters in READ_DATA)
    static const int QUALITY_WINDOW = 50;
    int m_quality_buf[QUALITY_WINDOW]; // ring buffer of process_bytes() return values
    int m_quality_head;                // next write position
    int m_quality_count;               // number of valid entries (0..QUALITY_WINDOW)
